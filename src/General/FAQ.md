...
- The legacy (-nvidia) image supports Pascal, Maxwell, and Volta architectures (GTX 900, GTX 1000, Nvidia Titan V, GTX 750 (TI) and 745)
...
There are currently no plans to support Kepler and older architectures. They are **no longer supported** by the upstream Nvidia drivers.
- You can still install Bazzite, using the open-source nouveau driver. However, the performance and stability of this driver lags behind the official Nvidia drivers.
- It is **not possible** to manually install the proprietary Nvidia drivers for your older card in Bazzite. You would need a different distribution that allows for such, if you wish to use the proprietary Nvidia drivers for your GPU.

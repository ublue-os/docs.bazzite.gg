...
1. [**Bazzite Portal (`ujust` Commands)**](./Bazzite_Portal.md) (_Convenience Commands_) - Custom scripts maintained by Bazzite & Universal Blue contributors that can also install a small subset of applications. <small>(This takes precedent over other formats but since it can only install only a small set of software, it is considered as an extension to system-level settings as opposed to an app store.)</small>
